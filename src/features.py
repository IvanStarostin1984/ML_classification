...
class FeatureEngineer:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_fe["emi_simple"] = df_fe["loan_amount"].fillna(loan_med) / df_fe[
            "loan_term"
        ].replace(0, np.nan).fillna(term_med)

        r = self.MARKET_APR / 12.0
        n = df_fe["loan_term"].replace(0, np.nan).fillna(term_med)
        P = df_fe["loan_amount"].fillna(loan_med)
        df_fe["emi_amortised"] = P * r * (1 + r) ** n / ((1 + r) ** n - 1 + 1e-6)

        df_fe["debt_to_income_ratio"] = df_fe["loan_amount"] / (
            df_fe["total_income_month"] * 12 + 1e-6
        )
        ...
        df_fe["income_per_dependent"] = df_fe["total_income_month"] / (
            df_fe["number_of_dependents"] + 1
        )

        df_fe["luxury_asset_ratio"] = df_fe["luxury_assets_value"] / (
            df_fe["total_assets"] + 1e-6
        )
        liquid_assets = df_fe["bank_asset_value"].fillna(0) + df_fe[
            "residential_assets_value"
        ].fillna(0)
        ...
        df_fe["selfemp_loan_to_income"] = df_fe["self_employed_flag"] * (
        ...
        if cat_cols:
            df_fe = pd.get_dummies(
                df_fe, columns=cat_cols, prefix_sep="=", drop_first=True
            )
